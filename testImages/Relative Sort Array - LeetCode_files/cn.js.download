...
div.winner img.simp-avatar[alt="liouzhou_101"]::after {
  content: "";
  display: block;
}
</style>
...
function applyFlag(usName) {
  if (document.querySelector("div.winner img.simp-avatar") == null) {
    setTimeout(function() {
      applyFlag(usName);
    }, 100);
  } else if (document.querySelector(`div.winner img.simp-avatar[alt="${usName}"]`) != null) {
    var avatarGot = document.querySelector(`div.winner img.simp-avatar[alt="${usName}"]`);
    avatarGot.outerHTML = `${avatarGot.outerHTML}<div class="cn_flag_moji" style="position:relative;top:16px;right:-33px;font-size:33px;">🇨🇳</div>`;
  }
}